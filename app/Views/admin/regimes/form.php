...
<head>
    ...
    <title><?= $mode === 'edit' ? 'Modifier' : 'Ajouter' ?> un regime</title>
    ...
    <div class="page">
        <div class="container">
            ...
            <div class="card form-card fade-up">
                <h1 class="card-title"><?= $mode === 'edit' ? 'Modifier un regime' : 'Ajouter un regime' ?></h1>

                <?php
                    $prixBaseValue = old('prix_base') ?: ($regime['prix_base'] ?? '');
                    $dureeValue = old('duree') ?: ($regime['duree'] ?? '');
                    $computedPrixParJour = '';
                    if ($prixBaseValue !== '' && $dureeValue !== '' && (float) $dureeValue > 0) {
                        $computedPrixParJour = number_format((float) $prixBaseValue / (float) $dureeValue, 2, '.', '');
                    }
                    $prixParJourValue = old('prix_par_jour') ?: ($computedPrixParJour !== '' ? $computedPrixParJour : ($regime['prix_par_jour'] ?? ''));
                ?>
                ...
                <form class="form-grid" action="<?= $mode === 'edit' ? site_url('/admin/regimes/update/' . $regime['id_regime']) : site_url('/admin/regimes/store') ?>" method="post">
                    <div class="field">
                    ...
                    <div class="field">
                        <label for="prix_par_jour">Prix par jour (calcule)</label>
                        <input type="number" step="0.01" id="prix_par_jour" name="prix_par_jour" value="<?= esc($prixParJourValue) ?>" readonly>
                    </div>
                    ...
                    <div class="actions">
                        ...
                        <a class="btn btn-ghost" href="<?= site_url('/admin/regimes') ?>">Retour</a>
                    </div>
                </form>
            </div>
